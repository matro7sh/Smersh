...
export class VulnsComponent implements OnInit {
  ...
  constructor(private vulnsServices: VulnsService, private router: Router) {
    ...
    this.vulnsServices.getData().subscribe((events) => {
      events['hydra:member'].forEach((el) => {
        if (this.currentLocal === 'fr') {
          this.vulns.push({
            id: el['id'],
            name: el['translations']['fr'].name,
            description: el['translations']['fr'].description,
            remediation: el['translations']['fr'].remediation,
          });
        } else {
          this.vulns.push({
            id: el['id'],
            name: el['translations']['en'].name,
            description: el['translations']['en'].description,
            remediation: el['translations']['en'].remediation,
          });
        }
      });
